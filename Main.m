...
% Initialisation 
load dbTreasuryCurve.mat                              % Load dataset : weekly US yield rates for different maturities
N=size(ParFitted,2);                                  % ParFitted = Yield curve
YieldChanges=ParFitted(2:end,:)-ParFitted(1:end-1,:); % "Returns"/ yield change
CurrentYields=ParFitted(end,:);                       % Last yield

%% Our data
Datenum=datestr(Dates,'mm-yy')
plot(Dates,ParFitted) 
Maturities= ["3 Months","2 Years","10 Years","20 Years","30 Years"]
datetick('x','mm-yy')
title("US Yield curve Time series")
xlabel("Time interval")
ylabel("Yield Rate")
legend("3 Months","2 Years","10 Years","20 Years","30 Years")
%% 3D representation
...
ylabel("Time interval")
zlabel("Yield Rate")
%% linear dependance of the data : the furthest the yields are appart from each other, the lesser they behave LINEARLY.
